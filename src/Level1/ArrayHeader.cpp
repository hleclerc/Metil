...
// if dim == 1
template<int want_machine_id>
inline MO *init_dyn_array_kernel( MO &res, const ST *size, Number<1> n_dim, Type *base_type, const MachineId *mid, Number<want_machine_id> wmi ) {
    ST al, ss;
    if ( base_type ) {
        ...
        if ( want_machine_id ) {
            if ( mid->is_a_gpu() )
                res.type = &metil_type_bas_Array_4NULL_1_m_m_CptUse_Gpu;
            else
                res.type = &metil_type_bas_Array_4NULL_1_m_m_CptUse_MachineId;
        } else
            res.type = &metil_type_bas_Array_4NULL_1_m_m_CptUse;
    }

    // rese_mem
    typedef Level1::ArrayHeader</*size*/1,/*rese*/1,/*CptUse*/1,/*ExtPtr*/0,want_machine_id,/*wrong alignment_in_bytes*/1> AH;
    ST real_hea = ceil_p2( sizeof( AH ), al );
    ST rese_mem = real_hea + ss * size[ 0 ];
    PRINT( rese_mem );
    ...
    header->rese[ 0 ] = ( rese_mem - real_hea ) / ss;
...
inline MO *init_dyn_array_kernel( MO &res, const ST *size, Number<dim> n_dim, Type *base_type, const MachineId *mid ) {
    if ( mid != MachineId::cur() )
        return init_dyn_array_kernel( res, size, n_dim, base_type, mid, Number<1>() );
    return init_dyn_array_kernel( res, size, n_dim, base_type, mid, Number<0>() );
}
