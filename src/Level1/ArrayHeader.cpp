...
inline MO *init_dyn_array_kernel( MO &res, const ST *size, Number<1> n_dim, Type *base_type, const MachineId *mid, Number<want_ExtPtr> wep, Number<want_MachineId> wmi ) {
    ...
    if ( base_type ) {
        ...
        res.type = base_type->dyn_array_type( n_dim, mid );
    } else {
        ASSERT( mid->is_a_gpu() == false, "Creation an untyped Array on a GPU is unusefull." );
        al = 1;
        ss = sizeof( MO );
        if ( want_machine_id ) {
            res.type = &metil_type_bas_Array_4NULL_1_m_m_CptUse_ExtPtr_MachineId;
        } else
            res.type = &metil_type_bas_Array_4NULL_1_m_m_CptUse;
    }

    // rese_mem
    typedef Level1::ArrayHeader</*size*/1,/*rese*/1,/*CptUse*/true,want_ExtPtr,want_MachineId,/*wrong alignment_in_bytes*/1> AH;
